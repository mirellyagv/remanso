            ...
            <div class="card">
                <div class="accordion accordion-flush modified-accordion" id="accordionFlushExample">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2 mb-3">
                                        <input type="text" class="form-control form-remanso align-right"
                                            name="fchIni" id="fchIni" placeholder="seleccione..">
                                    </div>
                                    <div class="col-md-1 mb-3">
                                        <label for="inputText" class="col-form-label">Fecha Fin: </label>
                                    </div>
                                    <div class="col-md-2 mb-3">
                                        <input type="date" class="form-control form-remanso align-right"
                                            name="fchFin" id="fchFin" placeholder="seleccione..">
                                    </div>
                                    <div class="col-md-1 mb-3">
                                    ...
                                    <div class="col-2 mb-3 d-none d-md-block" style="text-align: end">
                                        <button class="btn btn-secondary form-remanso" id="excelBtn1"
                                            onclick="BuscarProspecto()">Buscar
                    ...
                    @if (session('flg_jefe') === 'SI' ||
                            session('flg_supervisor') === 'SI' ||
                            session('flg_administrador') === 'SI' ||
                            session('flg_sac') === 'SI') col-md-2 mb-3
                    @else
                      col-md-3 mb-3 @endif 
                    ">
                                        <input type="text" class="form-control form-remanso align-right"
                    ...
                    @if (session('flg_jefe') === 'SI' ||
                            session('flg_supervisor') === 'SI' ||
                            session('flg_administrador') === 'SI' ||
                            session('flg_sac') === 'SI') col-md-1 mb-3
                    @else
                      col-md-2 mb-3 @endif 
                    ">
                    ...
                    @else
                      col-md-3 mb-3 @endif 
                    ">
                                        <input type="text" class="form-control form-remanso" name="nombreProspecto"
                                            id="nombreProspecto">
                                    </div>
                                    @if (session('flg_jefe') === 'SI' ||
                                            session('flg_supervisor') === 'SI' ||
                                        ...
                                        <div class="col-md-2 mb-3">
                                            <input type="text" class="form-control form-remanso" name="buscaVendedor"
                                                id="buscaVendedor">
                                        </div>
                                    @endif
                                    <div class="col-1 mb-3 d-md-none d-block">
                                        <button class="btn btn-secondary form-remanso" id="excelBtn1"
                                            onclick="BuscarProspecto()">Buscar &nbsp;<span
                                                class="bi bi-search"></span></button>
                                    </div>
                                    <div class="col-md-2 mb-3 d-none d-md-block" style="text-align: end">
                                        <button class="btn btn-danger BtnverdeRemanso form-remanso"
                                            onclick="descargarTablaExcel()" id="btnExportar"><img
                                                src="{{ asset('images/icon-excel.svg') }}" alt="Bootstrap"
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var firstDayOfMonth = new Date(currentDate.getFullYear(), currentDate.getMonth(),
            1); // Crear las fechas de inicio y fin del mes actual
        var lastDayOfMonth = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0);

        var firstDayOfMonthStr = firstDayOfMonth.toISOString().split('T')[
            0]; // Formatear las fechas como cadenas en formato "YYYY-MM-DD"
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {
                'fch_inicio': $('#fchIni').val(),
                'fch_fin': $('#fchFin').val(),
                'cod_estado': $('#EstDoc').val(),
                'dsc_documento': $('#numDoc').val(),
                'dsc_prospecto': $('#nombreProspecto').val(),
                'cod_tipo_necesidad': cod_tipo_necesidad,
                'dscVendedor': $('#buscaVendedor').val()
            },
            success: function(respuesta) {
                ...
                respuesta['response'].forEach(function(word) {
                    ...
                    var fch_registro1 = fch1.toLocaleDateString('en-GB', {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric'
                    }).replace(/ /g, '-');
                    ...
                    var dscObservado = '';
                    if (estado == 'ACTIVO') {
                        ref = 'href="{{ route('prospectos.actualizar') }}?CodProspecto=' +
                            word['cod_prospecto'] + '"';
                        info = 'Gestión';
                        ref2 = 'href="{{ route('ventas.registro') }}?CodProspecto=' + word[
                            'cod_prospecto'] + '"';
                    } else if (estado == 'VENTA' || estado == 'CIERRE' || estado ==
                        'PRE-VENTA') {
                        if (estado == 'VENTA') {
                        ...
                        ref = '';
                        info =
                            'Este prospecto esta en un estado avanzado, solo se puede modificar registros con estado ACTIVOS.'
                        ref2 = 'href="{{ route('ventas.registro') }}?CodProspecto=' + word[
                            'cod_prospecto'] + '"';
                        ...
                        dscObservado =
                            '<br><span class="bi bi-exclamation-triangle" style="font-size:1.5em;" onclick="alertaObservacion(' +
                            observacion + ')"></span>';
                    } else {
                        ref = '';
                        info = '';
                    ...
                    fila += '<tr><td>' +
                        ...
                        '<a class="btn btn-secondary form-remanso" ' + ref + ' title="' +
                        info +
                        '"><span class="bi bi-clipboard-check" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Gestión"></span></a>' +
                        '@endif' +
                        '<a class="btn btn-danger BtnverdeRemanso form-remanso" ' + ref2 +
                        ' title="Registrar venta"><span class="bi bi-cash-stack"></span></a>' +
                        '@if (session('flg_jefe') === 'SI' || session('flg_administrador') === 'SI')' +
                        '<button @if ('+estado+' != 'TRUNCO' || '+estado+' != 'CADUCO') disabled @endif class="btn btn-warning form-remanso" onclick="cambiarEdoP(' +
                        codProsp + ',' + dscProsp + ',' + estado1 +
                        ');" id="cambiaEdo" title="Activar Prospecto"><span class="bi bi-bookmark-star"></span></button>@endif</td>' +
                        '<td>' + word['cod_prospecto'] + '</td>' +
                        '<td style="text-align: left;" >' + word['dsc_tipo_documento'] +
                        '-' + word['dsc_documento'] + '</td>' +
                        ...
                        '<td ' + color + '><b>' + word['dsc_estado'] + dscObservado +
                        '</b></td>' +
                        '</tr>';
                ...
                $('#totalRegistros').text(
                    totalRegistros); // Actualiza el contador de totales en la tabla
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                ...
                respuesta['response'].forEach(function(word) {
                    ...
                    color = '';
                    if (estado == 'ACTIVO') {
                        ref = 'href="{{ route('prospectos.actualizar') }}?CodProspecto=' + word[
                            'cod_prospecto'] + '"';
                        ref2 = 'href="{{ route('ventas.registro') }}?CodProspecto=' + word[
                            'cod_prospecto'] + '"';
                    ...
                    fila += '<tr><td>' +
                        '@if (session('flg_nf') === 'SI' || session('flg_administrador') === 'SI' || session('flg_sac') === 'SI')' +
                        '<a class="btn btn-secondary form-remanso" ' + ref + ' title="' + info +
                        '"><span class="bi bi-clipboard-check" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Gestión"></span></a>' +
                        '@endif' +
                        '<a class="btn btn-danger BtnverdeRemanso form-remanso" ' + ref2 +
                        '><span class="bi bi-cash-stack" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Registrar venta"></span></a>' +
                        '@if (session('flg_jefe') === 'SI' || session('flg_administrador') === 'SI')' +
                        '<button class="btn btn-warning form-remanso" @if ('+estado+' === 'VENTA' || '+estado+' === 'ACTIVO') disabled @endif  onclick="cambiarEdoP(' +
                        codProsp + ',' + dscProsp + ',' + estado1 +
                        ');" id="cambiaEdo" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Registrar venta"><span class="bi bi-bookmark-star"></span></button>@endif</td>' +
                        '<td>' + word['cod_prospecto'] + '</td>' +
                        '<td>' + word['dsc_tipo_documento'] + '-' + word['dsc_documento'] +
                        '</td>' +
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                ...
                if (respuesta['response'].length > 0) {
                    data = [];
                    var header = ['CODIGO', 'DOCUMENTO', 'NOMBRES Y APELLIDOS DEL PROSPECTO',
                        'TIPO NECESIDAD', 'FECHA DE REGISTRO', 'DIAS', 'CANAL', 'VENDEDOR', 'ESTADO'
                    ];
