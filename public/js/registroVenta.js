window.onload = function () {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (respuesta) {
            $("#tipoDocRegVta").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
            );
            $("#tipoDoc2doRegVta").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
            );
            $("#tipoDocAval").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
            );
            $("#tipoDocAddBenef").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
            );
            respuesta["response"].forEach(function (word) {
                seleccion = "";
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (respuesta) {
            $("#edoCivilAddBenef").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
            );
            $("#edoCivilRegVta").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
            );
            $("#edoCivil2doRegVta").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
            );
            $("#edoCivilAval").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
    ...
    var flg_ni = botonTNec.checked ? "SI" : "NO";
    var flg_nf = botonTNec.checked ? "NO" : "SI";
    $.ajax({
        url: "../lista/MuestraTipoRecaudacion",
        method: "GET",
        crossDomain: true,
        dataType: "json",
        data: { flg_ni: flg_ni, flg_nf: flg_nf },
        success: function (respuesta) {
    ...
    $.ajax({
        ...
        success: function (respuesta) {
            $("#codCuotaServ").append(
                '<option value="000" selected disabled>SELECCIONE...</option>'
            );
            respuesta["response"].forEach(function (word) {
                seleccion = "";
                $("#codCuotaServ").append(
...
coddptto.addEventListener("change", function () {
    ...
    $.ajax({
        ...
        data: {
            cod_pais: codPais1,
            cod_departamento: coddptto1,
            cod_provincia: codprov1,
        },
        success: function (respuesta) {
            $("#dttoRegVta").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
...
codTipoProg.addEventListener("change", function () {
    ...
    if (validaEsp == "NO") {
        ...
        if (esCompart == "SI") {
            ...
            document
                .getElementById("nivelRegVnta")
                .setAttribute("disabled", "disabled");
            var optionsNivel = document.querySelectorAll(
                "#nivelRegVnta option"
            );
    ...
    if (flgIntg == "SI") {
        ...
        document
            .getElementById("tipoServicio")
            .setAttribute("disabled", "disabled");
        document.getElementById("subtipoServ").value = "";
        document
            .getElementById("subtipoServ")
            .setAttribute("disabled", "disabled");
    } else {
...
codcampo.addEventListener("change", function () {
    ...
    $.ajax({
        ...
        data: {
            ...
            cod_tipo_plataforma: codTipoplat,
        },
        success: function (respuesta) {
            $("#nombrePlat").append(
                '<option value="" selected disabled>SELECCIONE...</option>'
    ...
    $.ajax({
        ...
        success: function (respuesta) {
            if (respuesta["response"] == "") {
                $("#espacio").append(
                    '<option value="" selected disabled>NO HAY ESPACIOS DIPONIBLES</option>'
                );
            } else {
                $("#espacio").append(
                    '<option value="" selected disabled>SELECCIONE...</option>'
...
codEspacio.addEventListener("change", function () {
    ...
    $.ajax({
        ...
        data: {
            ...
            espacio: espacio,
            tipo: tipoEspacio,
            cod_trabajador: codConsejero,
        },
        success: function (respuesta) {
            if (
                respuesta["response"]["flg_acceso"] == "SI" ||
                (respuesta["response"]["flg_acceso"] == "NO" &&
                    (flg_supervisor == "SI" ||
                        flg_jefe == "SI" ||
                        flg_firmante == "SI" ||
                        flg_administrador == "SI"))
            ) {
                validaSepa = "SI";
                document
...
codcampo.addEventListener("click", function () {
    ...
    var selectedOptionTrec =
        tipoProgramaSelect.options[tipoProgramaSelect.selectedIndex];
    ...
    $.ajax({
        ...
        data: {
            cod_camposanto: codCamposanto,
            cod_plataforma: codPlataforma,
            cod_tipo_recaudacion: cod_tipo_recaudacion,
            cod_subtipo_servicio: cod_subtipo_servicio,
            sintegral: integralValue,
            tipo_nec: tipo_nec,
        },
        success: function (respuesta) {
...
function muestraserviciosFormulario(datos) {
    ...
    if (
        document.getElementById("tieneDS").value == "SI" &&
        datos["flg_dsepultura"] == "SI"
    ) {
        Swal.fire({
            title: "Error!",
    ...
    codigoCelda.textContent =
        datos["cod_servicio"] + " " + datos["dsc_servicio"];
    ...
    var valorLibre = datos["imp_descuento_adicional"]
        ? datos["imp_descuento_adicional"]
        : 0;
    ...
    btnEliminarInput.addEventListener("click", function () {
        eliminarFilaServicios(nuevaFilaServ);
    });
    ...
    dsctoPorcInput.addEventListener("input", function () {
        ...
        serviciosAgregados[nuevaFilaServ.rowIndex - 1]["por_descuento"] =
            dsctoPorcInput.value;
        recalcularSuma();
    ...
    dsctoLibreInput.addEventListener("input", function () {
        ...
        // document.getElementById("impCuoi").value=Number(datos['imp_precio_cuoi']).toFixed(2);
        cuoIniInput.dispatchEvent(changeEvent);
        serviciosAgregados[nuevaFilaServ.rowIndex - 1][
    ...
    document.getElementById("impPrecioLista").value = Number(
        datos["imp_precio_lista"]
    ).toFixed(2);
    ...
    cuoIniInput.addEventListener("input", function () {
        ...
        serviciosAgregados[nuevaFilaServ.rowIndex - 1]["imp_cui"] =
            cuoIniInput.value;
    ...
    function calculaSaldo(ctd, dsctoLibre, dsctoPorc, precio) {
        ...
        serviciosAgregados[nuevaFilaServ.rowIndex - 1]["imp_dscto"] =
            dscto - dsctoLibre;
        serviciosAgregados[nuevaFilaServ.rowIndex - 1]["imp_cui"] =
            cuoIniInput.value;
        serviciosAgregados[nuevaFilaServ.rowIndex - 1]["imp_saldo"] =
            precioFinal1Celda.textContent;
        document.getElementById("impDsctoAdicional").value =
            Number(dsctoLibre).toFixed(2);
    ...
    datosXserv = {
        ...
        imp_descuento_adicional: datos["imp_descuento_adicional"]
            ? datos["imp_descuento_adicional"]
            : 0, //validar como string
        imp_cui_minimo: datos["imp_min_cuoi"],
    };
    inputInteres.dispatchEvent(changeEvent2);
    cantInput.dispatchEvent(changeEvent);
    cuoIniInput.dispatchEvent(changeEvent);
...
function recalcularSuma() {
    ...
    for (let i = 0; i < tbody.length; i++) {
        const precioFinalCelda =
            tbody[i].getElementsByTagName("td")[7].textContent;
        ...
        const precioFinalSaldo =
            tbody[i].getElementsByTagName("td")[9].textContent;
...
//             console.log("Servicio:", serviciosAgregados[index - 1]);
//             console.log("num_linea:", serviciosAgregados[index - 1]?.num_linea);
...
function eliminarFilaServicios(fila) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            // console.log(serviciosAgregados[index]?.num_linea);

            const eliminarDelDOM = () => {
                const tbody = fila.parentNode;
                tbody.removeChild(fila);

                const fomaLinea = serviciosAgregados[index]["imp_foma"];
                fomaTotal = fomaTotal - parseFloat(fomaLinea);
                document.getElementById("impFoma").value = fomaTotal.toFixed(2);

                serviciosAgregados.splice(index, 1);

                const inputIntegral = document.getElementById("tieneDS");
                if (inputIntegral.value == "SI") {
                    inputIntegral.value = "NO";
                }
                ...
                const CuotaInicialInput = document.getElementById("impCuoi");
                const changeEvent = new Event("input");
                CuotaInicialInput.dispatchEvent(changeEvent);

                Swal.fire({
                    icon: "success",
                    html: "Servicio eliminado<br><br>correctamente.",
            ...
            if (serviciosAgregados[index]["num_linea"] != 0) {
                const codProsp = serviciosAgregados[index]["cod_prospecto"];
                const numlinea = serviciosAgregados[index]["num_linea"];
                ...
                $.ajax({
                    ...
                    error(e) {
                        ...
                        Swal.fire({
                            title: "Error",
                            text: "No se pudo eliminar el servicio del sistema. Intente nuevamente.",
                            icon: "error",
                            confirmButtonColor: "#35B44A",
                        });
                    },
...
btnAbreModalBenef.addEventListener("click", function () {
    document
        .getElementById("btnUpdBeneficiario")
...
selectComprobante.addEventListener("change", function () {
    if (selectComprobante.value == "TC001") {
        document
            .getElementById("rucCompVtaRegVta")
            .setAttribute("disabled", "disabled");
        document.getElementById("rucCompVtaRegVta").value = "";
        document
            .getElementById("razSocCompVtaRegVta")
            .setAttribute("disabled", "disabled");
